...
def _send_pings(aap2_client, destination, interval, count, stop_event,
                timeout):
...
def _try_receive_ping(aap2_client, logger):
    ...
    # AAP2Client.receive will return None, e.g., if uD3TN disconnects.
    if not msg:
...
def run_aap_ping(rpc_client, sub_client, destination, interval, count,
                 logger, timeout):
    ...
    send_worker = threading.Thread(
        target=_send_pings,
        args=(rpc_client, destination, interval, count, stop_event, timeout),
    )
    send_worker.start()
